...
public class VwJobs extends org.jooq.impl.TableImpl<com.sonicle.webtop.drm.jooq.tables.records.VwJobsRecord> {
    ...
    private VwJobs(org.jooq.Name alias, org.jooq.Table<com.sonicle.webtop.drm.jooq.tables.records.VwJobsRecord> aliased, org.jooq.Field<?>[] parameters) {
        super(alias, null, aliased, parameters, org.jooq.impl.DSL.comment(""), org.jooq.TableOptions.view("create view \"vw_jobs\" as SELECT jb.job_id, jb.operator_id, us.display_name AS operator_description, jb.customer_id, rmd.description AS customer_description, jb.customer_stat_id, smd.description AS customer_stat_description, jb.activity_id, ac.description AS activity_description, jb.start_date, jb.end_date, jb.timezone, jb.company_id, co.name AS company_description, jb.title, jb.ticket_id, jb.domain_id, jb.number, jb.description, jb.causal_id, ca.description AS causal_description FROM ((((((drm.jobs jb LEFT JOIN drm.companies co ON (((jb.company_id = co.company_id) AND ((jb.domain_id)::text = (co.domain_id)::text)))) LEFT JOIN core.master_data rmd ON ((((jb.customer_id)::text = (rmd.master_data_id)::text) AND ((jb.domain_id)::text = (rmd.domain_id)::text)))) LEFT JOIN core.master_data smd ON ((((jb.customer_stat_id)::text = (smd.master_data_id)::text) AND ((jb.domain_id)::text = (rmd.domain_id)::text)))) LEFT JOIN drm.activities ac ON (((jb.activity_id = ac.activity_id) AND ((jb.domain_id)::text = (ac.domain_id)::text)))) LEFT JOIN core.users us ON ((((jb.operator_id)::text = (us.user_id)::text) AND ((jb.domain_id)::text = (us.domain_id)::text)))) LEFT JOIN core.causals ca ON (((jb.causal_id = ca.causal_id) AND ((jb.domain_id)::text = (ca.domain_id)::text))));"));
    }
